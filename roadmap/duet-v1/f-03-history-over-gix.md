...
- `crates/duet-project/src/history/gix_impl.rs` — modify. Chunk F1 created the stub.
- `crates/duet-project/src/history/budget.rs` — modify.
- `crates/duet-project/Cargo.toml` — modify. Add the `gix` entry.
...
1. Read every file of the write scope and `crates/duet-project/Cargo.toml`. Confirm that chunk F1 left both files a stub and that chunk F2 has landed. Report a discrepancy and stop.
2. Add to `crates/duet-project/Cargo.toml` the entry `gix = { workspace = true }`. Run `cargo build --workspace` and keep `Cargo.lock` for the same commit.
...
6. One commit on the branch `chunk/f3-history-over-gix` passes the native git hook. The commit carries `crates/duet-project/Cargo.toml` and `Cargo.lock` together.
