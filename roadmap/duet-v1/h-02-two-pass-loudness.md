...
**This chunk writes no member manifest**, so it names neither `crates/duet-export/Cargo.toml` nor `Cargo.lock` in its write scope, and its Completion command carries no `--locked` flag (SM3 rule 4, SM5).

## Files

- `crates/duet-export/src/loudness/analyse.rs` — modify. Chunk H1 created the stub.
- `crates/duet-export/src/loudness/limit.rs` — modify.
