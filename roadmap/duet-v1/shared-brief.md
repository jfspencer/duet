...
## Repository state
`crates/bc_app/duet/lang_rust` is a skeleton (one window, one root view). `tools/bc_plan_store/plan-db/lang_rust` and `tools/bc_repo_guard/xtask/lang_rust` exist. Read `CLAUDE.md` for the lint policy (no unwrap, no panic, no indexing, no `as`, no prose `//` comments, every item documented), the test rules, and the Definition of Done. A new crate goes at `crates/bc_<context>/<crate>/lang_rust/` with `[lints] workspace = true` (ADR 0011).
